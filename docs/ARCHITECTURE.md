...
- **`next/image` поверх storage Fastify.** Картинку грузит браузер напрямую из
  `/static/**`, оптимизатор Next её прогоняет. Ключевое для интеграции:
  `remotePatterns` вместо устаревших `domains`, `qualities: [75]` обязателен (иначе
  любое другое `quality` приводится к ближайшему), а `dangerouslyAllowLocalIP` нужен
  ТОЛЬКО в dev — Next 16 по умолчанию блокирует локальные адреса, а storage висит на
  `localhost:3000`. В проде — реальный CDN-домен, флаг убирается. URL картинок
  абсолютные (их отдаёт Fastify), поэтому идут в `src` как есть.
- **`next/dynamic` c `ssr: false` — через клиентскую границу.** `ssr: false`
  запрещён в Server Component, поэтому ленивую загрузку панели заворачиваем в
  `'use client'`-обёртку. Панель завязана на браузерный WebSocket — серверный рендер
  ей не нужен, код уезжает в отдельный чанк.
- **React Compiler** стабилен, но на Babel (дольше сборка) — включаем осознанно;
  нарушителей Rules of React пропускает с предупреждением, а не ломает.
- **Сквозная трассировка в тот же Jaeger.** `@vercel/otel` авто-инструментирует
  `fetch` и пробрасывает W3C `traceparent`: спан Next связывается со спаном Fastify,
  тот добавляет Postgres. Оба экспортируют в один Jaeger → единый трейс Next →
  Fastify → Postgres. `serviceName` держим отличным от Fastify; клиентская
  телеметрия (`instrumentation-client.ts`) — отдельный слой (серверный трейс в
  браузер не тянем — упёрлись бы в CORS).
- **`global-error.tsx`** заменяет корневой layout при его падении — обязан включать
  `<html>`/`<body>` и быть Client Component.
- **Общий кеш через Redis (`cacheHandlers`).** С `cacheComponents` кеш по умолчанию
  in-memory и per-instance. Хендлер под новый интерфейс (`get`/`set`/`refreshTags`/
  `getExpiration`/`updateTags`) кладёт записи в общий Redis. Устойчивость: `get()`
  глушит ошибки и возвращает `undefined` (фреймворк НЕ оборачивает его в try/catch),
  `set()` — best-effort; при недоступном Redis приложение деградирует до «без общего
  кеша», а не падает. Грабля деплоя: с `output: 'standalone'` конфиг замораживается
  на сборке — гейтить хендлер по рантайм-`REDIS_URL` нельзя, поэтому включаем флагом
  `DOCKER_BUILD` на билде (заодно так `redis` трейсится в standalone). `.cjs`, потому
  что проект ESM, а хендлер грузится как CommonJS. Кросс-подовая координация тегов —
  предмет прод-доработки (`refreshTags`).
- **i18n — опциональный паттерн.** App Router не маршрутизирует локали из коробки;
  показан рабочий каркас (словари с ленивой подгрузкой + разбор `Accept-Language`)
  без перестройки маршрутов под `[lang]`.
- **Продакшен.** `output: 'standalone'` (тоже под флагом `DOCKER_BUILD`, иначе
  локальный `next start` ругается на standalone), многоступенчатый `Dockerfile`,
  подключение сервиса `web` в общий `docker-compose`. Суть — разделение
  internal/external адресов: `API_INTERNAL_URL=api:3000` (сервер Next внутри сети
  compose), `NEXT_PUBLIC_*=localhost` (браузер через опубликованный порт); `REDIS_URL`
  и OTLP-endpoint указывают на те же `redis` и `jaeger`, что у Fastify.
- **Герметичная сборка без Fastify.** При `docker compose up --build` (и в CI)
  образ собирается ДО старта сервисов — Fastify недоступен. Поэтому build-time
  обращения к API деградируют: `generateStaticParams` товара и `sitemap.ts` ловят
  ошибку и отдают заглушку/статические URL; страница управления выносит список в
  динамическую дыру через `await connection()` (не резолвится на prerender);
  `generateMetadata` заглушки `__placeholder__` не ходит в API. Товары дорисуются
  on-demand и ревалидацией в рантайме. Прикладные ошибки API (`ApiError`) при этом
  не маскируются — деградируем только на сетевой недоступности.
- **Тесты и CI.** Vitest — чистые функции (`format`, `i18n`); Playwright — сквозные
  сценарии против живого стека (после логина ждём редирект на `/account`, иначе
  переход оборвёт server action и сессия не сохранится). CI (GitHub Actions) гоняет
  типы, линт, юнит-тесты и сборку; `schema.d.ts` закоммичен, поэтому `gen:api`
  (нужен живой Fastify) в CI не запускается.
